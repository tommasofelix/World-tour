...
func _ready() -> void:
	...
	print("\n--------------------------------------------------------")
	print("ESITO COMPLESSIVO TEST MAPPA & CITTÀ (F8.1):")
	print("  Test Superati: %d" % tests_passed)
	print("  Test Falliti:  %d" % tests_failed)
	print("--------------------------------------------------------\n")
	
	if tests_failed == 0:
		print("[SUCCESSO] La Mappa Geografica & Sistema delle Città (F8.1) è convalidata al 100%!")
		get_tree().quit(0)
	else:
...
func test_city_data_catalog() -> void:
	print("\n--- TEST 1: CATALOGO CITTÀ ---")
	var cities: Array[CityData] = CityDataScript.get_all_cities()
	assert_equal(cities.size(), 6, "Il catalogo contiene esattamente 6 città (4 nazionali, 2 internazionali)")
...
func test_territorial_fans_and_reverberation() -> void:
	...
	# 85% vanno a Bologna (+85), 15% distribuiti sulle altre 5 città (+3 a testa: 15 / 5 = 3)
	...
	assert_equal(mi_fans, 103, "Milano riceve il riverbero (+3 fan = 103)")
	
	var ro_fans: int = travel_sys.get_fans_in_city(Enums.CityId.ROMA)
	assert_equal(ro_fans, 3, "Roma riceve il riverbero (+3 fan)")
	
	assert_equal(player.fans, 200, "Il totale dei fan su tutte le città ammonta esattamente a 200 (100 iniziali + 100 nuovi)")
