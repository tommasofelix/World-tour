...
- [ ] `F9.1F`: **La Band, Reclutamento, Dinamiche Relazionali & Revenue Split (Sezione 3)**: Evoluzione del quartetto (Basso, Batteria, Tastiere, Chitarra Ritmica), bacheca audizioni con domande attitudinali, affinità/rispetto/tensione, politiche di revenue split (Equa, Leader, Predatoria), rischio abbandono su crisi di gruppo, integrazione live/prove e accessibilità tastiera NVDA in `BandHub`. In preparazione (File di riferimento: `docs/roadmap/03_band_reclutamento_dinamiche_e_revenue_split.md`).
