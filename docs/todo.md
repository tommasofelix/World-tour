...
- [x] `F9.6`: **Espansione Post-V5.1: Endless Horizon, New Game+ Heirs, Circuito a 16 Metropoli & Magnate Discografico Attivo (Versione AVF V5.2.0)**: Modalità Carriera Infinita (Endless Sandbox) post-Last Waltz con avanzamento deterministico anno per anno (`current_year`); meccanica New Game+ (Legacy Heirs / Il Discepolo) con eredità strumento iconico, tratto unico `LEGACY_DISCIPLE` e rendita passiva del mentore (15 €/giorno); circuito espanso a 16 metropoli mondiali (San Paolo, Buenos Aires, Sydney, Seoul) con 4 nuovi festival estivi, tratte aeree transoceaniche e affinità territoriali; magnate discografico con produzione esecutiva attiva di album per le band del proprio roster (`produce_band_album`), scalata nelle classifiche Top 10 con tag dedicato e dividendi settimanali; nuova suite headless `test_endless_and_ngplus_system.gd` (67 test superati) e verifica globale al 100% su 28/28 suite con 0 errori e 0 ms.
- [x] `F9.7`: **Restyle Grafico & Accessibilità del Menu Principale (Versione AVF V5.3.0)**: Scena `main_menu.tscn` in pixel art retrò arcade 16/32-bit fedele al concept visuale; sfondo panoramico pulito `background_menu_clean.png` con folla viva (punk e jazzisti al crepuscolo), insegne neon e strada con pentagramma luminoso; logo `game_title.png`; 4 pulsanti neon verticali ("NUOVA PARTITA", "CARICA PARTITA", "IMPOSTAZIONI", "ESCI AL DESKTOP") con tema `menu_theme.tres` e font `PressStart2P.ttf`; integrazione atomica con `SaveManager.load_game()`; pattern Zero Focus Drop per accessibilità NVDA; navigazione ciclica continua da tastiera (Frecce, Tab, Numpad 8/2); nuova suite headless `test_main_menu.gd` (33 asserzioni) e verifica globale al 100% su 29/29 suite con 0 errori a 0 ms. Collaudo in-game approvato con esito eccezionale da Luca.
- [x] `F9.8`: **Gameplay Grafico 2.5D Isometrico, Loft NYC, Controller Alex, 10 Arredi Interattivi & Simmetria Universale (Versione AVF V5.4.0)**: Transizione da HUD a mondo esplorabile 2.5D isometrico nel loft di New York (`scenes/apartment/apartment.tscn`, `apartment.gd`); controller del protagonista Alex (`player_alex.tscn`, `player_alex.gd`) con 4 direzioni animate, hitbox ai piedi per clearance continua e bump audio protetto (<= 0.75f); 10 arredi interattivi concentrici con Y-sorting (Chitarra, Cucina, Divano, Giradischi, Arcade, Letto, Armadio, Cassa attrezzi, Porta NYC, Stereo da studio); interazione mouse Holy Diver con hover cursore a manina (`CURSOR_POINTING_HAND`), highlight e click sinistro con auto-walk al punto di sosta calpestabile (`stand_offset`), base per il futuro menu radiale stile *The Sims*; interazione Luca NVDA con ciclo Tab/Shift+Tab e Numpad 7/9 con vocalizzazione lineare e conservazione integrale dei 15 tasti rapidi HUD; ApartmentHud a schermo intero con barre vitali e router 19 modali; collisioni ricalibrate per tavolino e perimetro pulito; nuova suite `test_apartment_gameplay.gd` (68 asserzioni) e verifica globale 30/30 suite headless al 100% con 0 errori a 0 ms.
- [x] `F9.9`: **Revisione Interazioni, Geometria Loft, Arredi Domestici & Accessibilità (Versione AVF V5.4.1)**: Risoluzione deterministica delle 5 anomalie di gameplay nel loft: rimozione nodo `Prompt` fluttuante `[SPAZIO]` in `interactive_prop.tscn` e `.gd` (Contratto D0 Clean Sweep); riallineamento geometrico al millimetro delle radici dei 7 arredi disassati (`PropKitchen`, `PropCouch`, `PropTurntable`, `PropArcade`, `PropBed`, `PropWardrobe`, `PropToolbox`) alla base sul pavimento azzerando scostamenti anomali di 300px e stand offset calpestabili; logica di auto-walk in `player_alex.gd` con guardia di stallo che previene aperture spurie del menu a vuoto se Alex incontra un ostacolo; riparazione metodi temporali in `time_system.gd` (`advance_to_next_period`, `trigger_sleep_now`) e gestione del Letto (notte con sonno immediato, giorno con scelta lineare Z/X); differenziazione azioni domestiche (divano relax gratuito, cucina espresso casalingo, giradischi ascolto vinili e ispirazione) senza modale generica da bar/parco; stato bistabile a levetta On/Off per lo stereo (`is_stereo_on`) con annunci e inspection box dinamici. Suite `test_apartment_gameplay.gd` espansa a 84 asserzioni e 30/30 suite headless superate al 100% con 0 errori a 0 ms.

---

### ROADMAP DI ESPANSIONE (V2.0 – V5.4.1)
...
- [x] `V5.2.0`: Endless Horizon, New Game+ Heirs, 16 metropoli mondiali, magnate discografico attivo e 28 suite headless al 100% con 0 errori a 0 ms (completato e convalidato al 100%).
- [x] `V5.3.0`: Restyle grafico del Menu Principale in pixel art retrò arcade, 4 pulsanti neon, integrazione atomica Carica Partita, tema dedicato, Zero Focus Drop per NVDA e 29 suite headless al 100% con 0 errori a 0 ms (completato e convalidato al 100%).
- [x] `V5.4.0`: Gameplay Grafico 2.5D Isometrico, Loft NYC, Controller Alex, 10 arredi interattivi, mouse hover click a manina per Holy Diver (The Sims style), ciclo Tab/Numpad per NVDA, 19 modali integrate, ricalibrazione collisioni e 30 suite headless al 100% con 0 errori a 0 ms (completato e convalidato al 100%).
- [x] `V5.4.1`: Revisione Interazioni, Geometria Loft, Arredi Domestici, Stereo Toggle, Letto e Bonifica Prompt [SPAZIO] con 30 suite headless al 100% con 0 errori a 0 ms (completato e convalidato al 100%).
